      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          ...
          <Link to="/" className="flex items-center gap-3 group">
            <div className={`p-2.5 rounded-xl shadow-lg border border-brand-orange/20 transition-colors ${
              isScrolled || location.pathname !== '/' 
            ...
            <div className="flex flex-col">
              <span className={`font-tech font-bold text-xl tracking-tight leading-none ${
            ...
            <nav className="flex space-x-6">
              {navLinks.map((link) => (
                <Link
                  key={link.name}
                  to={link.href}
                  className={`text-sm font-medium transition-colors hover:text-brand-orange font-tech uppercase tracking-wide relative group ${
        ...
        <div className="px-4 py-4 space-y-3">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              to={link.href}
              className={`block px-4 py-3 rounded-lg text-base font-semibold transition-colors font-tech uppercase ${
                location.pathname === link.href ? 'bg-brand-orange text-white' : 'text-white hover:text-brand-orange hover:bg-brand-navyLight'
              }`}
            >
              {link.name}
            </Link>
